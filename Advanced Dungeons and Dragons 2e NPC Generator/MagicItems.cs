...
namespace NPCGen
{
    class MagicItems
    {
        ...
        public static POWER PowerByLevel(int Level, ref Random random)
        {
            switch (Level)
            {
                case 1:
                ...
                case 6:
                    if (Dice.Percentile(ref random) == 100)
                        return POWER.MEDIUM;
                    return POWER.MINOR;
                case 7:
                    if (Dice.Percentile(ref random) > 97)
                        return POWER.MEDIUM;
                    return POWER.MINOR;
                case 8:
                    if (Dice.Percentile(ref random) > 96)
                        return POWER.MEDIUM;
                    return POWER.MINOR;
                case 9:
                    if (Dice.Percentile(ref random) > 91)
                        return POWER.MEDIUM;
                    return POWER.MINOR;
                case 10:
                ...
                case 16:
                    ...
                    return POWER.MINOR;
                case 17:
                    if (Dice.Percentile(ref random) > 83)
                        return POWER.MAJOR;
                    return POWER.MEDIUM;
                case 18:
                    ...
                    return POWER.MEDIUM;
                case 19:
                    if (Dice.Percentile(ref random) > 70)
                        return POWER.MAJOR;
                    return POWER.MEDIUM;
                case 20:
                    if (Dice.Percentile(ref random) > 65)
                        return POWER.MAJOR;
                    return POWER.MEDIUM;
        ...
        public static string Generate(POWER Power, int Quantity, ref Random random)
        {
            string output = "";
        ...
        {
            string output = ""; bool SpecificCursed; int Roll;

            SpecificCursed = false;
            if (Dice.Percentile(ref random) <= 5)
                output += CursedItem.Generate(ref SpecificCursed, ref random);
            ...
            if (!SpecificCursed)
            {
                ...
                {
                    case POWER.MINOR:
                        Roll = Dice.Roll(1, 21, 0, ref random);
                        if (Roll > 19)
                            output += Ring(Power, ref random);
                        else if (Roll > 10)
                            output += WondrousItem(ref random, Power);
                        else
                            output += Wand(ref random, Power);
                        break;
                    case POWER.MEDIUM:
                        Roll = Dice.Roll(1, 55, 0, ref random);
                        if (Roll > 52)
                            output += Staff(Power, ref random);
                        else if (Roll > 42)
                            output += Ring(Power, ref random);
                        else if (Roll > 32)
                            output += Rod(Power, ref random);
                        else if (Roll > 17)
                            output += Wand(ref random, Power);
                        else
                            output += WondrousItem(ref random, Power);
                        break;
                    case POWER.MAJOR:
                        Roll = Dice.Roll(1, 65, 0, ref random);
                        if (Roll > 45)
                            output += Staff(Power, ref random);
                        else if (Roll > 35)
                            output += Ring(Power, ref random);
        ...
        public static string Potion(POWER Power, int Quantity, ref Random random)
        {
            string output = "";
            ...
            switch (Power)
            {
                ...
                case POWER.MAJOR:
                    switch (Dice.Percentile(ref random))
                    {
                        ...
                        case 82:
                        case 83:
                        case 84: return String.Format("ring of spell storing ({0}, {1})", Classes.WizardMemorizedSpell(random.Next(1, 10), ref random), SpecialQualities(false, 6, ref random));
                        case 85:
                        case 86:
        ...
        private static string Wand(ref Random random, POWER Power)
        {
            switch (Power)
            {
                case POWER.MINOR:
        ...
        private static string WondrousItem(ref Random random, POWER Power)
        {
            switch (Power)
            {
                case POWER.MINOR:
                    switch (Dice.Roll(1, 98, 0, ref random))
                    {
                        ...
                        default: return WondrousItem(ref random, POWER.MEDIUM);
                    }
                case POWER.MEDIUM:
                    switch (Dice.Roll(1, 99, 0, ref random))
                    {
                        ...
                        case 38: return String.Format("ioun stone (vibrant purple prism, {0}, {1})", Classes.WizardMemorizedSpell(random.Next(1, 10), ref random), SpecialQualities(false, 3, ref random));
                        ...
                        case 100: return WondrousItem(ref random, POWER.MAJOR);
                ...
                case POWER.MAJOR:
                    switch (Dice.Percentile(ref random))
                    {
                        ...
                        case 100: return WondrousItem(ref random, POWER.MAJOR) + " and " + WondrousItem(ref random, POWER.MAJOR);
