...
namespace NPCGen
{
    class Scrolls
    {
        ...
        public static string Generate(Classes.CLASS Class, int Level, int Quantity, ref Random random)
        {
            MagicItems.POWER Power = MagicItems.PowerByLevel(Level, ref random);
            string output = "";
            SPELLTYPE SpellType = SpellTypeByClass(Class, ref random);

            while (Quantity > 0)
            {
                if (output != "")
                    output += "\n";
                output += "Scroll " + SpellType.ToString() + ": " + Spells(SpellType, Power, ref random);
                Quantity--;
            }
        ...
        private static SPELLTYPE SpellTypeByClass(Classes.CLASS Class, ref Random random)
        {
            switch (Class)
            {
                ...
                case Classes.CLASS.RANGER: return SPELLTYPE.DIVINE;
                case Classes.CLASS.BARD:
                case Classes.CLASS.SORCERER:
                case Classes.CLASS.WIZARD: return SPELLTYPE.ARCANE;
                default:
        ...
        {
            return (SPELLTYPE)random.Next(1, 3);
        }
