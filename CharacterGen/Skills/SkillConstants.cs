namespace CharacterGen.Skills
{
    public static class SkillConstants
    {
        public const string Appraise = "Appraise";
        public const string Balance = "Balance";
        public const string Bluff = "Bluff";
        public const string Climb = "Climb";
        public const string Concentration = "Concentration";
        public const string Craft = "Craft";
        public const string DecipherScript = "Decipher Script";
        public const string Diplomacy = "Diplomacy";
        public const string DisableDevice = "Disable Device";
        public const string Disguise = "Disguise";
        public const string EscapeArtist = "Escape Artist";
        public const string Forgery = "Forgery";
        public const string GatherInformation = "Gather Information";
        public const string HandleAnimal = "Handle Animal";
        public const string Heal = "Heal";
        public const string Hide = "Hide";
        public const string Intimidate = "Intimidate";
        public const string Jump = "Jump";
        public const string Knowledge = "Knowledge";
        public const string Listen = "Listen";
        public const string MoveSilently = "Move Silently";
        public const string OpenLock = "Open Lock";
        public const string Perform = "Perform";
        public const string Profession = "Profession";
        public const string Ride = "Ride";
        public const string Search = "Search";
        public const string SenseMotive = "Sense Motive";
        public const string SleightOfHand = "Sleight of Hand";
        public const string Spellcraft = "Spellcraft";
        public const string Spot = "Spot";
        public const string Survival = "Survival";
        public const string Swim = "Swim";
        public const string Tumble = "Tumble";
        public const string UseMagicDevice = "Use Magic Device";
        public const string UseRope = "Use Rope";

        public static class Foci
        {
            public const int QuantityOfAll = int.MaxValue;

            public static class Craft
            {
                public const string Alchemy = "Alchemy";
                public const string Armorsmithing = "Armorsmithing";
                public const string Blacksmithing = "Blacksmithing";
                public const string Bookbinding = "Bookbinding";
                public const string Bowmaking = "Bowmaking";
                public const string Brassmaking = "Brassmaking";
                public const string Brewing = "Brewing";
                public const string Candlemaking = "Candlemaking";
                public const string Cloth = "Cloth";
                public const string Coppersmithing = "Coppersmithing";
                public const string Dyemaking = "Dyemaking";
                public const string Gemcutting = "Gemcutting";
                public const string Glass = "Glass";
                public const string Goldsmithing = "Goldsmithing";
                public const string Hatmaking = "Hatmaking";
                public const string Hornworking = "Hornworking";
                public const string Jewelmaking = "Jewelmaking";
                public const string Leather = "Leather";
                public const string Locksmithing = "Locksmithing";
                public const string Mapmaking = "Mapmaking";
                public const string Milling = "Milling";
                public const string Painting = "Painting";
                public const string Parchmentmaking = "Parchmentmaking";
                public const string Pewtermaking = "Pewtermaking";
                public const string Potterymaking = "Potterymaking";
                public const string Sculpting = "Sculpting";
                public const string Shipmaking = "Shipmaking";
                public const string Shoemaking = "Shoemaking";
                public const string Silversmithing = "Silversmithing";
                public const string Skinning = "Skinning";
                public const string Soapmaking = "Soapmaking";
                public const string Stonemasonry = "Stonemasonry";
                public const string Tanning = "Tanning";
                public const string Trapmaking = "Trapmaking";
                public const string Weaponsmithing = "Weaponsmithing";
                public const string Weaving = "Weaving";
                public const string Wheelmaking = "Wheelmaking";
                public const string Winemaking = "Winemaking";
                public const string Woodworking = "Woodworking";
            }

            public static class Knowledge
            {
                public const string Arcana = "Arcana";
                public const string ArchitectureAndEngineering = "Architecture and Engineering";
                public const string Dungeoneering = "Dungeoneering";
                public const string Geography = "Geography";
                public const string History = "History";
                public const string Local = "Local";
                public const string Nature = "Nature";
                public const string NobilityAndRoyalty = "Nobility and Royalty";
                public const string Religion = "Religion";
                public const string ThePlanes = "The Planes";
            }

            public static class Perform
            {
                public const string Act = "Act";
                public const string Comedy = "Comedy";
                public const string Dance = "Dance";
                public const string KeyboardInstruments = "Keyboard instruments";
                public const string Oratory = "Oratory";
                public const string PercussionInstruments = "Percussion instruments";
                public const string StringInstruments = "String instruments";
                public const string WindInstruments = "Wind instruments";
                public const string Sing = "Sing";
            }

            public static class Profession
            {
                public const string Adviser = "Adviser";
                public const string Alchemist = "Alchemist";
                public const string AnimalGroomer = "Animal Groomer";
                public const string AnimalTrainer = "Animal Trainer";
                public const string ExoticAnimalTrainer = "Exotic Animal Trainer";
                public const string Apothecary = "Apothecary";
                public const string Appraiser = "Appraiser";
                public const string Architect = "Architect";
                public const string Armorer = "Armorer";
                public const string Barrister = "Barrister";
                public const string Blacksmith = "Blacksmith";
                public const string Bookbinder = "Bookbinder";
                public const string Bowyer = "Bowyer";
                public const string Brazier = "Brazier";
                public const string Brewer = "Brewer";
                public const string Butler = "Butler";
                public const string Carpenter = "Carpenter";
                public const string Cartographer = "Cartographer";
                public const string Cartwright = "Cartwright";
                public const string Chandler = "Chandler";
                public const string Clerk = "Clerk";
                public const string Cobbler = "Cobbler";
                public const string Coiffeur = "Coiffeur";
                public const string Coffinmaker = "Coffinmaker";
                public const string Cook = "Cook";
                public const string LocalCourier = "Local Courier";
                public const string OutOfTownCourier = "Out-of-Town Courier";
                public const string Coppersmith = "Coppersmith";
                public const string Dowser = "Dowser";
                public const string Dyer = "Dyer";
                public const string Embalmer = "Embalmer";
                public const string Engineer = "Engineer";
                public const string Entertainer = "Entertainer";
                public const string Farmer = "Farmer";
                public const string Fletcher = "Fletcher";
                public const string Footman = "Footman";
                public const string Gemcutter = "Gemcutter";
                public const string Goldsmith = "Goldsmith";
                public const string Governess = "Governess";
                public const string WildernessGuide = "Wilderness Guide";
                public const string CityGuide = "City Guide";
                public const string Haberdasher = "Haberdasher";
                public const string Healer = "Healer";
                public const string Horner = "Horner";
                public const string Hunter = "Hunter";
                public const string Interpreter = "Interpreter";
                public const string Jeweler = "Jeweler";
                public const string Laborer = "Laborer";
                public const string Craftsman = "Craftsman";
                public const string Launderer = "Launderer";
                public const string Limner = "Limner";
                public const string Locksmith = "Locksmith";
                public const string Maid = "Maid";
                public const string Masseuse = "Masseuse";
                public const string Matchmaker = "Matchmaker";
                public const string Midwife = "Midwife";
                public const string Miller = "Miller";
                public const string Navigator = "Navigator";
                public const string Nursemaid = "Nursemaid";
                public const string Painter = "Painter";
                public const string Parchmentmaker = "Parchmentmaker";
                public const string Pewterer = "Pewterer";
                public const string Polisher = "Polisher";
                public const string Porter = "Porter";
                public const string Potter = "Potter";
                public const string Sage = "Sage";
                public const string SailorCrewmember = "Sailor Crewmember";
                public const string SailorMate = "Sailor Mate";
                public const string Scribe = "Scribe";
                public const string Sculptor = "Sculptor";
                public const string Shepherd = "Shepherd";
                public const string Shipwright = "Shipwright";
                public const string Silversmith = "Silversmith";
                public const string Skinner = "Skinner";
                public const string Soapmaker = "Soapmaker";
                public const string Soothsayer = "Soothsayer";
                public const string Tanner = "Tanner";
                public const string Teacher = "Teacher";
                public const string Teamster = "Teamster";
                public const string Trader = "Trader";
                public const string Valet = "Valet";
                public const string Vintner = "Vintner";
                public const string Weaponsmith = "Weaponsmith";
                public const string Weaver = "Weaver";
                public const string Wheelwright = "Wheelwright";
            }
        }
    }
}