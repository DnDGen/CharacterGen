...
namespace NPCGen.Tests.Unit.Selectors
{
    ...
    {
        private ILanguagesSelector provider;
        private Race race;
        private Mock<ILanguagesMapper> mockLanguagesXmlParser;
        ...
        public void Setup()
        {
            ...
            mockLanguagesXmlParser = new Mock<ILanguagesMapper>();
            mockLanguagesXmlParser.Setup(p => p.Parse(It.IsAny<String>())).Returns(parsedLanguages);

            provider = new LanguagesSelector(mockLanguagesXmlParser.Object);
        }
